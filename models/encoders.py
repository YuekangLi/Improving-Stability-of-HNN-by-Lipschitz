...
#from layers_2 import _variable_on_device
...
class Encoder(nn.Module):
    ...
    def encode(self, x, adj):
        ...
        if self.encode_graph:
            ...
            output, drop_weight = self.layers.forward(x)
...
class HNN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        for i in range(len(dims) - 1):
            ...
            #hnn_layers.append(
            #        nn.LayerNorm(
            #            [183, 16], eps=1e-05, elementwise_affine=True)
            #)
    ...
    def encode(self, x, adj):
        ...
        #x_proj=self.manifold.proj_tan0(x, self.c)
        #print(x_proj)
        #print(x_proj.shape)

        #x_exp=self.manifold.expmap0(self.manifold.proj_tan0(x, self.c), c=self.c)
        #print(x_exp)
        #print(x_exp.shape)
        ...
        noise1 = torch.normal(0,1,size=x.shape).cuda()
        noise01 = torch.normal(0,0.1,size=x.shape).cuda()
        noise001 = torch.normal(0,0.01,size=x.shape).cuda()
        noise0001 = torch.normal(0,0.001,size=x.shape).cuda()
        noise0005 = torch.normal(0,0.005,size=x.shape).cuda()
        
        noise=noise0005
        #np.savetxt('./cha1_noise001.csv',noise.detach().numpy(),fmt='%.6f',delimiter=',')
        #print(noise.shape)
        #print(noise)
        #print('noise')
        
        #expn=self.manifold.expmap0(noise,c=self.c)
        #ptan=self.manifold.ptransp0(expn,x,c=self.c)
        #x_hyp = self.manifold.proj(self.manifold.expmap(ptan,expn ,c=self.c), c=self.c)
        #a=(x_hyp)[:x_hyp.shape[0]-1000]
        #print(a)
        #b=(x_hyp+noise)[x_hyp.shape[0]-1000:]
        #print(b)
        #print(a.shape)
        #print(b.shape)
        #x_hyp=torch.cat([a,b],dim=0)
        #x_hyp=x_hyp+noise
        ...
        #print(super(HNN, self).encode(x_hyp, adj).shape)
        #x_hnn=super(HNN, self).encode(x_hyp, adj)
        #print(x_hnn.shape)
        #print(x_hnn)
        #print('x_hnn')
        output, drop_weight =super(HNN, self).encode(x_hyp, adj)

        return output, drop_weight, x_hyp
